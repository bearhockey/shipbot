...
partial-mode GIFs containing the occasional full-frame redraw, which would results
...
'''
I made this so I can use it. Fucking command line scripts blow ass.
'''
...
def process_image(path, frame=0):
    # Iterate the GIF, extracting X frame.
    ...
    try:
        print "saving %s (%s) frame %d, %s %s" % (path, mode, frame, im.size, im.tile)
